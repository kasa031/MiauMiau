...
let gameState = {
    ...
    currentCat: 0,
    activeCats: [0], // Array of cat indices that are currently active (friends)
    dailyChallenge: null,
    challengeProgress: 0,
...
function loadGame() {
    ...
    try {
        ...
        if (!gameState.activeCats || gameState.activeCats.length === 0) {
            gameState.activeCats = [gameState.currentCat || 0];
        }
...
function installPWA() {
    ...
    if (isStandalone()) {
        showMessage('Appen er allerede installert! 🎉');
        return;
    }
...
// ==================== CAT FRIENDS SYSTEM ====================
function openCatFriendsOverlay() {
    const overlay = document.getElementById('cat-friends-overlay');
    if (!overlay) return;
    
    overlay.style.display = 'flex';
    renderCatFriendsGrid();
}

function closeCatFriendsOverlay() {
    const overlay = document.getElementById('cat-friends-overlay');
    if (overlay) {
        overlay.style.display = 'none';
    }
...
function renderCatFriendsGrid() {
    const grid = document.getElementById('cat-friends-grid');
    if (!grid) return;
    
    grid.innerHTML = '';
    
    gameState.cats.forEach((cat, index) => {
        const isActive = gameState.activeCats.includes(index);
        const isUnlocked = cat.unlocked;
        const canAdd = gameState.activeCats.length < 5;
        
        const card = document.createElement('div');
        card.className = `cat-friend-card ${isActive ? 'active' : ''} ${!isUnlocked ? 'locked' : ''}`;
        
        let imageHtml = '';
        if (cat.image && isUnlocked) {
            const imageClass = cat.isGif ? 'cat-friend-image cat-friend-gif' : 'cat-friend-image';
            imageHtml = `<img src="${cat.image}" alt="${cat.name}" class="${imageClass}">`;
        } else if (cat.image && !isUnlocked) {
            imageHtml = `<img src="${cat.image}" alt="${cat.name}" class="cat-friend-image locked">`;
        }
        
        card.innerHTML = `
            <div class="cat-friend-info">
                ${imageHtml}
                <div class="cat-friend-emoji">${cat.emoji}</div>
                <div class="cat-friend-name">${cat.name}</div>
                ${!isUnlocked ? `<div class="cat-friend-locked">🔒 Låst</div>` : ''}
                ${isActive ? '<div class="cat-friend-active">✓ Aktiv</div>' : ''}
            </div>
            <button class="action-btn cat-friend-btn" 
                    onclick="toggleCatFriend(${index})" 
                    ${!isUnlocked ? 'disabled' : ''}
                    ${isActive && gameState.activeCats.length <= 1 ? 'disabled title="Du må ha minst én aktiv katt"' : ''}
                    ${!isActive && !canAdd ? 'disabled title="Maks 5 katter kan være aktive samtidig"' : ''}>
                ${isActive ? '❌ Fjern' : '➕ Legg til'}
            </button>
        `;
...
function toggleCatFriend(catIndex) {
    if (!gameState.cats[catIndex] || !gameState.cats[catIndex].unlocked) {
        showMessage(t('catNotUnlocked') || 'Katten er ikke låst opp ennå!');
        return;
    }
    
    const isActive = gameState.activeCats.includes(catIndex);
    
    if (isActive) {
        // Remove cat if there's more than one active
        if (gameState.activeCats.length > 1) {
            gameState.activeCats = gameState.activeCats.filter(idx => idx !== catIndex);
            showMessage(`${gameState.cats[catIndex].name} er fjernet fra aktive katter.`);
        } else {
            showMessage(t('mustHaveOneActiveCat') || 'Du må ha minst én aktiv katt!');
            return;
        }
    } else {
        // Add cat if under limit
        if (gameState.activeCats.length < 5) {
            gameState.activeCats.push(catIndex);
            showMessage(`${gameState.cats[catIndex].name} er lagt til som aktiv katt! 🐱`);
        } else {
            showMessage(t('maxActiveCats') || 'Du kan maksimalt ha 5 katter aktive samtidig!');
            return;
        }
    }
...
function renderActiveCats() {
    const container = document.getElementById('active-cats-container');
    if (!container) return;
    
    // Ensure activeCats array exists and has at least one cat
    if (!gameState.activeCats || gameState.activeCats.length === 0) {
    ...
    if (gameState.activeCats.length === 0) {
        container.innerHTML = '<p style="color: #999;">Ingen aktive katter. Legg til katter i kattevenner-panelet!</p>';
        return;
    }
    
    gameState.activeCats.forEach(catIndex => {
        const cat = gameState.cats[catIndex];
        if (!cat) return;
        
        const catCard = document.createElement('div');
        catCard.className = 'active-cat-card';
        catCard.dataset.catIndex = catIndex;
        
        let imageHtml = '';
        if (cat.image && cat.unlocked) {
            const imageClass = cat.isGif ? 'active-cat-image active-cat-gif' : 'active-cat-image';
            imageHtml = `<img src="${cat.image}" alt="${cat.name}" class="${imageClass}">`;
        }
        ...
        catCard.innerHTML = `
            <div class="active-cat-header">
                ${imageHtml}
                <div class="active-cat-emoji">${cat.emoji}</div>
                <div class="active-cat-name">${cat.name}</div>
            </div>
            <div class="active-cat-stats">
                <div class="active-cat-stat">
            ...
            <button class="action-btn active-cat-interact-btn" onclick="interactWithCat(${catIndex})" style="margin-top: 10px; font-size: 0.9em;">
                Interager med ${cat.name}
            </button>
        `;
...
function interactWithCat(catIndex) {
    // Set this cat as the current cat for interactions
    gameState.currentCat = catIndex;
    updateStats();
    renderActiveCats();
    showMessage(`Du interagerer nå med ${gameState.cats[catIndex].name}! 🐱`);
}

// Event listener for manage cats button
